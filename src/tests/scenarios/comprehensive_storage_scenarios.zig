...
test "scenario: partial WAL write during crash" {
    ...
    var runner = try SimulationRunner.init(
        ...
        operation_mix,
        &.{
            .partial_write_probability = 0.1, // 10% chance of partial writes
        },
    );
    defer runner.deinit();
...
test "scenario: I/O errors during write operations" {
    ...
    var runner = try SimulationRunner.init(
        ...
        operation_mix,
        &.{
            .io_error_probability = 0.05, // 5% I/O error rate
        },
    );
    defer runner.deinit();
...
test "scenario: checksum failures during read operations" {
    ...
    var runner = try SimulationRunner.init(
        ...
        operation_mix,
        &.{
            .corruption_probability = 0.02, // 2% corruption rate
        },
    );
    defer runner.deinit();
...
test "scenario: sustained mixed workload for 10K operations" {
    ...
    var runner = try SimulationRunner.init(
        ...
        operation_mix,
        &.{
            .io_error_probability = 0.001, // Rare I/O errors
            .corruption_probability = 0.0001, // Very rare corruption
        },
    );
    defer runner.deinit();
...
test "scenario: regression - block iterator memory leak" {
    ...
    const final_memory = runner.memory_stats();
    try testing.expect(final_memory.total_allocated <= initial_memory.total_allocated * 1.5);
}
