...
test "memtable flush triggers at size threshold" {
    ...
    // Fill memtable close to threshold
    const block_size = 1024; // 1KB per block
    const content = try allocator.alloc(u8, block_size);
    defer allocator.free(content);
    @memset(content, 'A');

    var blocks_inserted: u32 = 0;
    const max_blocks = config.memtable_max_size / block_size;

    for (0..max_blocks + 1) |i| {
        const block_id = BlockId.from_bytes([_]u8{0} ** 15 ++ [1]u8{@intCast(i % 256)});
        const block = ContextBlock{
            .id = block_id,
            .version = 1,
            .source_uri = "file://large_test.zig",
            .metadata_json = "{}",
            .content = content,
        };
        ...
        if (engine.sstable_manager.total_block_count() > 0) {
            break;
        }
    }
...
test "WAL recovery restores storage state" {
    ...
    {
        var engine = try StorageEngine.init_default(allocator, sim_vfs.vfs(), "/test/recovery");
        defer engine.deinit();
    ...
    // Phase 2: Recovery
    {
        ...
        try engine.startup(); // Triggers WAL recovery

        // Verify blocks were recovered
        for (test_blocks) |expected_block| {
            const recovered_block = try engine.find_block(expected_block.id, .storage_engine);
            try testing.expect(recovered_block != null);
            try testing.expectEqualStrings(expected_block.content, recovered_block.?.read(.storage_engine).content);
        }
    }
}
...
test "graph edge operations work correctly" {
    ...
    var engine = try StorageEngine.init_default(allocator, sim_vfs.vfs(), "/test/edges");
    defer engine.deinit();

    try engine.startup();

    // Create blocks
    const source_id = BlockId.from_bytes([_]u8{1} ** 16);
    const target_id = BlockId.from_bytes([_]u8{2} ** 16);

    const source_block = ContextBlock{
        .id = source_id,
        .version = 1,
        .source_uri = "file://source.zig",
        .metadata_json = "{}",
    ...
    // Test edge retrieval
    const outgoing_edges = engine.find_outgoing_edges(source_id);
    try testing.expect(outgoing_edges.len > 0);
    try testing.expect(outgoing_edges[0].edge.target_id.eql(target_id));

    const incoming_edges = engine.find_incoming_edges(target_id);
    try testing.expect(incoming_edges.len > 0);
    try testing.expect(incoming_edges[0].edge.source_id.eql(source_id));
}
