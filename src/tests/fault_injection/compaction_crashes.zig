...
test "recovery from partial sstable write" {
    ...
    try scenarios.run_compaction_crash_scenario(allocator, .partial_sstable_write);
}
...
test "recovery with orphaned files" {
    ...
    try scenarios.run_compaction_crash_scenario(allocator, .orphaned_files);
}
...
test "multiple sequential crash recovery" {
    ...
    try scenarios.run_compaction_crash_scenario(allocator, .sequential_crashes);
}
...
test "torn write recovery" {
    ...
    try scenarios.run_compaction_crash_scenario(allocator, .torn_write_header);
}
...
test "systematic compaction crash validation" {
    ...
    try scenarios.run_all_compaction_scenarios(allocator);
}
