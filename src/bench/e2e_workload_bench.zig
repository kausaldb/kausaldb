...
const E2EWorkloadBench = struct {
    ...
    pub fn run_benchmark(self: *Self) !BenchmarkResults {
        ...
        try self.execute_timed_command(&[_][]const u8{"ping"}, &ingestion_sampler);
        try self.execute_timed_command(&[_][]const u8{"status"}, &ingestion_sampler);
        ...
        while (op_idx < self.config.operations_per_type) : (op_idx += 1) {
            ...
            try self.execute_timed_command(&[_][]const u8{ "find", func_name }, &find_sampler);
        }
        ...
        while (op_idx < self.config.operations_per_type) : (op_idx += 1) {
            ...
            try self.execute_timed_command(&[_][]const u8{ "show", "callers", file_name }, &show_sampler);
        }
        ...
        while (op_idx < self.config.operations_per_type) : (op_idx += 1) {
            ...
            try self.execute_timed_command(&[_][]const u8{ "trace", "main", func_name }, &trace_sampler);
        }

        return BenchmarkResults{
            .connectivity = ingestion_sampler.calculate_percentiles(),
...
pub fn run_e2e_benchmark_with_harness(harness: anytype) !void {
    ...
    try harness.add_result(BenchmarkResult{
        ...
        .mean_ns = results.connectivity.p50_ns, // Use median as mean for robust comparison
