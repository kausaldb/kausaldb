...
fn get_pid_file_path(allocator: std.mem.Allocator, port: u16) ![]u8 {
    return try std.fmt.allocPrint(allocator, "/tmp/kausaldb-{}.pid", .{port});
}

/// Global server reference for signal handler access
...
const KausalServer = struct {
    ...
    pub fn startup(self: *KausalServer) !void {
        ...
        // Convert data directory to absolute path for VFS compatibility
            ...
            for (ready_connections) |connection| {
                self.handle_connection_request(connection) catch |err| switch (err) {
                    ConnectionHandlingError.ConnectionClosed => {
                        self.connection_manager.?.close_connection_by_pointer(connection) catch |close_err| {
...
fn install_signal_handler(server: *KausalServer) !void {
    ...
    switch (builtin.os.tag) {
        .windows => {
            // Windows uses different signal handling mechanism
            return;
        },
        else => {
            ...
            // Also install handler for SIGINT (Ctrl+C) for foreground mode
...
fn write_pid_file(allocator: std.mem.Allocator, port: u16, pid: std.posix.pid_t) !void {
    const pid_file_path = try get_pid_file_path(allocator, port);
...
fn read_pid_file(allocator: std.mem.Allocator, port: u16) !?std.posix.pid_t {
    ...
    const max_pid_len = 32; // More than enough for any reasonable PID
...
fn remove_pid_file(allocator: std.mem.Allocator, port: u16) void {
    const pid_file_path = get_pid_file_path(allocator, port) catch return;
    defer allocator.free(pid_file_path);
