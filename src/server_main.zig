...
fn run_server_main(allocator: std.mem.Allocator) !ExitCode {
    ...
    switch (command) {
        ...
        .start => {
            ...
            const current_pid = blk: {
                const builtin = @import("builtin");
                switch (builtin.os.tag) {
                    .linux => break :blk std.os.linux.getpid(),
                    else => {
                        // Fallback PID for development - use timestamp-based ID
                        const timestamp = std.time.milliTimestamp();
                        break :blk @as(std.posix.pid_t, @intCast(@mod(timestamp, 65535) + 1000));
                    },
                }
            };
