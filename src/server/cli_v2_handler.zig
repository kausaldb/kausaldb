//! CLI v2 protocol handler for KausalDB server.
//!
//! Handles client requests from the new thin CLI client, translating between
//! CLI v2 wire protocol and internal engine APIs. Integrates with existing
...
const log = std.log.scoped(.cli_v2_handler);

/// CLI v2 request handler context
...
/// Handle CLI v2 protocol messages and return response bytes
pub fn handle_cli_v2_message(
    ctx: HandlerContext,
    message_type: cli_v2_protocol.MessageType,
    payload: []const u8,
) ![]const u8 {
    ...
    switch (message_type) {
        ...
        else => return try create_error_response(ctx, @intFromEnum(cli_v2_protocol.ErrorCode.unknown_command), "Unknown command"),
    }
}

fn handle_ping_request(ctx: HandlerContext) ![]const u8 {
    const response_header = cli_v2_protocol.MessageHeader{
        .message_type = .pong_response,
        .payload_size = 0,
    };

    const response_bytes = try ctx.allocator.alloc(u8, @sizeOf(cli_v2_protocol.MessageHeader));
...
fn handle_status_request(ctx: HandlerContext) ![]const u8 {
    ...
    const status = cli_v2_protocol.StatusResponse{
    ...
    const response_header = cli_v2_protocol.MessageHeader{
        ...
        .payload_size = @sizeOf(cli_v2_protocol.StatusResponse),
    };

    const total_size = @sizeOf(cli_v2_protocol.MessageHeader) + @sizeOf(cli_v2_protocol.StatusResponse);
    const response_bytes = try ctx.allocator.alloc(u8, total_size);

    @memcpy(response_bytes[0..@sizeOf(cli_v2_protocol.MessageHeader)], std.mem.asBytes(&response_header));
    @memcpy(response_bytes[@sizeOf(cli_v2_protocol.MessageHeader)..], std.mem.asBytes(&status));

    return response_bytes;
}

fn handle_find_request(ctx: HandlerContext, payload: []const u8) ![]const u8 {
    if (payload.len != @sizeOf(cli_v2_protocol.FindRequest)) {
        return try create_error_response(ctx, @intFromEnum(cli_v2_protocol.ErrorCode.invalid_request), "Invalid find request size");
    }

    const request = @as(*const cli_v2_protocol.FindRequest, @ptrCast(@alignCast(payload.ptr))).*;
    ...
    const semantic_result = ctx.query_engine.find_by_name("", "function", query_text) catch |err| {
        ...
        return try create_error_response(ctx, @intFromEnum(cli_v2_protocol.ErrorCode.server_error), error_msg);
    };
    ...
    // Build response
    var response = cli_v2_protocol.FindResponse.init();
    for (blocks.items) |block| {
        if (response.block_count >= cli_v2_protocol.MAX_BLOCKS_PER_RESPONSE) break;
...
fn handle_show_callers_request(ctx: HandlerContext, payload: []const u8) ![]const u8 {
    if (payload.len != @sizeOf(cli_v2_protocol.ShowRequest)) {
        return try create_error_response(ctx, @intFromEnum(cli_v2_protocol.ErrorCode.invalid_request), "Invalid show request size");
    }

    const request = @as(*const cli_v2_protocol.ShowRequest, @ptrCast(@alignCast(payload.ptr))).*;
...
fn handle_trace_request(ctx: HandlerContext, payload: []const u8) ![]const u8 {
    if (payload.len != @sizeOf(cli_v2_protocol.TraceRequest)) {
        return try create_error_response(ctx, @intFromEnum(cli_v2_protocol.ErrorCode.invalid_request), "Invalid trace request size");
    }

    const request = @as(*const cli_v2_protocol.TraceRequest, @ptrCast(@alignCast(payload.ptr))).*;
...
fn handle_link_request(ctx: HandlerContext, payload: []const u8) ![]const u8 {
    if (payload.len != @sizeOf(cli_v2_protocol.LinkRequest)) {
        return try create_error_response(ctx, @intFromEnum(cli_v2_protocol.ErrorCode.invalid_request), "Invalid link request size");
    }

    const request = @as(*const cli_v2_protocol.LinkRequest, @ptrCast(@alignCast(payload.ptr))).*;
...
fn handle_unlink_request(ctx: HandlerContext, payload: []const u8) ![]const u8 {
    if (payload.len != @sizeOf(cli_v2_protocol.SyncRequest)) {
        return try create_error_response(ctx, @intFromEnum(cli_v2_protocol.ErrorCode.invalid_request), "Invalid unlink request size");
    }

    const request = @as(*const cli_v2_protocol.SyncRequest, @ptrCast(@alignCast(payload.ptr))).*;
    const name_text = request.name_text();
...
fn handle_sync_request(ctx: HandlerContext, payload: []const u8) ![]const u8 {
    if (payload.len != @sizeOf(cli_v2_protocol.SyncRequest)) {
        return try create_error_response(ctx, @intFromEnum(cli_v2_protocol.ErrorCode.invalid_request), "Invalid sync request size");
    }
...
fn serialize_find_response(ctx: HandlerContext, response: cli_v2_protocol.FindResponse) ![]const u8 {
    const response_header = cli_v2_protocol.MessageHeader{
        .message_type = .find_response,
        .payload_size = @sizeOf(cli_v2_protocol.FindResponse),
    };

    const total_size = @sizeOf(cli_v2_protocol.MessageHeader) + @sizeOf(cli_v2_protocol.FindResponse);
    const response_bytes = try ctx.allocator.alloc(u8, total_size);

    @memcpy(response_bytes[0..@sizeOf(cli_v2_protocol.MessageHeader)], std.mem.asBytes(&response_header));
    @memcpy(response_bytes[@sizeOf(cli_v2_protocol.MessageHeader)..], std.mem.asBytes(&response));

    return response_bytes;
}

fn serialize_show_response(ctx: HandlerContext, blocks: []const ContextBlock, edges: []const GraphEdge) ![]const u8 {
    var response = cli_v2_protocol.ShowResponse.init();
    ...
    for (edges) |edge| {
        if (response.edge_count >= cli_v2_protocol.MAX_EDGES_PER_RESPONSE) break;
        response.add_edge(edge);
    }

    const response_header = cli_v2_protocol.MessageHeader{
        .message_type = .show_response,
        .payload_size = @sizeOf(cli_v2_protocol.ShowResponse),
    };

    const total_size = @sizeOf(cli_v2_protocol.MessageHeader) + @sizeOf(cli_v2_protocol.ShowResponse);
    const response_bytes = try ctx.allocator.alloc(u8, total_size);
...
fn serialize_empty_show_response(ctx: HandlerContext) ![]const u8 {
    const response = cli_v2_protocol.ShowResponse.init();
...
fn serialize_empty_trace_response(ctx: HandlerContext) ![]const u8 {
    const response = cli_v2_protocol.TraceResponse.init();

    const response_header = cli_v2_protocol.MessageHeader{
        .message_type = .trace_response,
        .payload_size = @sizeOf(cli_v2_protocol.TraceResponse),
    };

    const total_size = @sizeOf(cli_v2_protocol.MessageHeader) + @sizeOf(cli_v2_protocol.TraceResponse);
    const response_bytes = try ctx.allocator.alloc(u8, total_size);
...
fn serialize_operation_response(ctx: HandlerContext, success: bool, message: []const u8) ![]const u8 {
    const response = cli_v2_protocol.OperationResponse.init(success, message);

    const response_header = cli_v2_protocol.MessageHeader{
        .message_type = .operation_response,
        .payload_size = @sizeOf(cli_v2_protocol.OperationResponse),
    };

    const total_size = @sizeOf(cli_v2_protocol.MessageHeader) + @sizeOf(cli_v2_protocol.OperationResponse);
    const response_bytes = try ctx.allocator.alloc(u8, total_size);
...
fn create_error_response(ctx: HandlerContext, error_code: u32, message: []const u8) ![]const u8 {
    const error_resp = cli_v2_protocol.ErrorResponse.init(error_code, message);

    const response_header = cli_v2_protocol.MessageHeader{
        .message_type = .error_response,
        .payload_size = @sizeOf(cli_v2_protocol.ErrorResponse),
    };

    const total_size = @sizeOf(cli_v2_protocol.MessageHeader) + @sizeOf(cli_v2_protocol.ErrorResponse);
    const response_bytes = try ctx.allocator.alloc(u8, total_size);

    @memcpy(response_bytes[0..@sizeOf(cli_v2_protocol.MessageHeader)], std.mem.asBytes(&response_header));
    @memcpy(response_bytes[@sizeOf(cli_v2_protocol.MessageHeader)..], std.mem.asBytes(&error_resp));
