...
pub const ConnectionManager = struct {
    ...
    pub fn close_connection(self: *ConnectionManager, connection_ptr: *const ClientConnection) !void {
        for (self.connections.items, 0..) |existing_conn, index| {
            if (existing_conn == connection_ptr) {
                self.close_connection_by_index(index);
                return;
            }
        }
    ...
    pub fn cleanup_timed_out_connections(self: *ConnectionManager) !void {
        ...
        while (i < self.connections.items.len) {
            ...
            if (connection_age > timeout_seconds) {
                ...
                self.close_connection_by_index(i);
    ...
    pub fn process_connection_io(
        ...
        if (!keep_alive) {
            log.debug("Connection {} marked for closure", .{connection.connection_id});
            self.close_connection(connection) catch |err| {
                log.err("Failed to close connection {}: {}", .{ connection.connection_id, err });
            };
            return false;
        }
