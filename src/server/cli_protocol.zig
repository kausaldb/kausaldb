...
fn handle_status_request(ctx: HandlerContext) ![]const u8 {
    ...
    status.uptime_seconds = 0; // TODO: Implement proper server uptime tracking
...
fn handle_find_request(ctx: HandlerContext, payload: []const u8) ![]const u8 {
    ...
    std.log.debug("Find request: query_text='{s}', workspace='{s}', entity_type='{s}', name='{s}'", .{ query_text, workspace, entity_type, name });
