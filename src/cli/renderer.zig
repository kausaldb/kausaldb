...
fn render_status_text(ctx: *RenderContext, status: *const protocol.StatusResponse, verbose: bool) !void {
    // Format uptime and total storage
    ...
    if (workspaces.len > 0) {
        ...
        for (workspaces) |*workspace| {
            ...
            std.log.debug("Client: Received workspace name='{s}' sync_status={}", .{ name, @intFromEnum(workspace.sync_status) });
            ...
            // Color the status icon based on sync status
    ...
    // Verbose mode: show developer metrics
    if (verbose) {
        try ctx.write("\n");
