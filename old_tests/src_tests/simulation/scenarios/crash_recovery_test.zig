...
test "scenario: crash with I/O errors during recovery" {
    ...
    // Combine crash with I/O errors to test recovery robustness
    const faults = [_]FaultSpec{
        .{ .operation_number = 100, .fault_type = .crash },
        .{ .operation_number = 150, .fault_type = .io_error }, // I/O error during recovery
        .{ .operation_number = 250, .fault_type = .crash }, // Another crash
    };

    var runner = try SimulationRunner.init(allocator, 0x10E99, .{
        .put_block_weight = 80,
    ...
    log.info("Testing crash with I/O errors during recovery", .{});

    // Generate moderate structure
    var generator = CodePatternGenerator.init(allocator, runner.storage_engine, 0x10E99);
    defer generator.deinit();
    try generator.generate_monolithic_blocks(8);

    // Track initial state
    const initial_blocks = runner.storage_engine.metrics().blocks_written.load();
    log.info("Initial blocks before crash+I/O errors: {}", .{initial_blocks});

    // Run through crash and I/O error scenarios
    try runner.run(300);

    // System should handle cascading failures gracefully
    try PropertyChecker.check_no_data_loss(&runner.model, runner.storage_engine);

    log.info("Crash with I/O errors scenario completed successfully", .{});
}
