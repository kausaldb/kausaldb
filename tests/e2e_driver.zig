...
// Dynamic port allocation - no fixed port to avoid collisions
...
fn find_available_port() !u16 {
    // Try to bind to port 0 to let the OS allocate an available port
    const address = std.net.Address.parseIp("127.0.0.1", 0) catch return error.InvalidAddress;

    var listener = std.net.Address.listen(address, .{}) catch return error.CannotBindPort;
    defer listener.deinit();

    const bound_address = listener.listen_address;
    return bound_address.getPort();
}
...
pub fn main() !void {
    ...
    const server_result = try start_server_process(allocator);
    var server_process = server_result.process;
    const test_port = server_result.port;
    ...
    const cleanup_result = cleanup: {
        ...
        try wait_for_server_ready(test_port);
        std.debug.print("Server ready. Running E2E tests...\n\n", .{});

        const test_exit_code = run_test_suite(allocator, filter_arg, test_port) catch |err| {
...
fn kill_existing_server() void {
    ...
    // With dynamic port allocation, no need to kill processes on specific ports
    // Just kill any existing kausal processes to ensure clean state
...
fn start_server_process(allocator: std.mem.Allocator) !struct { process: ServerProcess, port: u16 } {
    // Find an available port dynamically
    const test_port = try find_available_port();
    ...
    // Use static paths to avoid memory corruption issues
    const stdout_log_path = TEST_DATA_DIR ++ "/server_stdout.log";
    const stderr_log_path = TEST_DATA_DIR ++ "/server_stderr.log";

    const server_process = ServerProcess{
        .process = process,
        .allocator = allocator,
    ...
    // Skip threading for now - just redirect to files directly
    // This eliminates all threading complexity and memory corruption issues
    return .{ .process = server_process, .port = test_port };
}
...
fn consume_server_output(server_process: *ServerProcess) void {
    // Open log files once at the start to avoid path corruption issues
    const stdout_file = std.fs.cwd().createFile(server_process.stdout_log_path, .{}) catch |err| {
        std.debug.print("WARNING: Could not create stdout log file: {}\n", .{err});
        return;
    };
    defer stdout_file.close();

    const stderr_file = std.fs.cwd().createFile(server_process.stderr_log_path, .{}) catch |err| {
        std.debug.print("WARNING: Could not create stderr log file: {}\n", .{err});
        return;
    };
    ...
    // Drain both pipes and log to files
    while (std.time.milliTimestamp() - start_time < timeout_ms) {
        var data_available = false;

        // Read and log stdout
        if (server_process.process.stdout) |stdout| {
            const bytes_read = stdout.read(&buffer) catch 0;
            if (bytes_read > 0) {
                stdout_file.writeAll(buffer[0..bytes_read]) catch {};
                data_available = true;
            }
        }
        ...
        if (server_process.process.stderr) |stderr| {
            const bytes_read = stderr.read(&buffer) catch 0;
            if (bytes_read > 0) {
...
fn drain_pipe_to_file(pipe: std.fs.File, log_path: []const u8) void {
    ...
    // Validate path before using it - production grade means checking inputs
    for (log_path) |c| {
        if (c == 0) {
            std.debug.print("ERROR: Null character found in log path, skipping logging\n", .{});
            return;
        }
    }

    // Open log file for writing with proper error handling
    const log_file = std.fs.cwd().createFile(log_path, .{}) catch |err| {
        std.debug.print("ERROR: Failed to create log file '{}': {}\n", .{ std.zig.fmtEscapes(log_path), err });
        return;
    };
...
fn wait_for_server_ready(port: u16) !void {
...
fn run_test_suite(allocator: std.mem.Allocator, filter_arg: ?[]const u8, test_port: u16) !u8 {
