...
test "sync all codebases at once" {
    ...
    var link1_result = try test_harness.execute_workspace_command("link --path {s}", .{project1_path});
    defer link1_result.deinit();
    try link1_result.expect_success();

    var link2_result = try test_harness.execute_workspace_command("link --path {s}", .{project2_path});
    defer link2_result.deinit();
    try link2_result.expect_success();

    // Sync projects
    var sync_all_result = try test_harness.execute_command(&[_][]const u8{ "sync", "--all" });
    defer sync_all_result.deinit();
    try sync_all_result.expect_success();
    try testing.expect(sync_all_result.stdout.len > 0 or sync_all_result.exit_code == 0);
}
