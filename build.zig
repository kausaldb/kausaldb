...
pub fn build(b: *std.Build) void {
    ...
    // Git hooks management (copies files to standard .git/hooks/ location)
    const hooks_install_step = b.step("hooks-install", "Install project git hooks to .git/hooks/");
    const hooks_install_cmd = b.addSystemCommand(&.{ "sh", "-c", "mkdir -p .git/hooks && cp .githooks/pre-commit .githooks/commit-msg .githooks/pre-push .git/hooks/ && chmod +x .git/hooks/pre-commit .git/hooks/commit-msg .git/hooks/pre-push && git config --unset core.hooksPath || true && echo 'Project hooks installed to .git/hooks/'" });
    ...
    const hooks_uninstall_cmd = b.addSystemCommand(&.{ "sh", "-c", "rm -f .git/hooks/pre-commit .git/hooks/commit-msg .git/hooks/pre-push && git config --unset core.hooksPath || true && echo 'Project hooks removed from .git/hooks/'" });
